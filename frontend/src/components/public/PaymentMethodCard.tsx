  ...
  return (
    <article className="flex flex-col rounded-card border border-navy/8 bg-white p-7 shadow-e2">
      <div className="mb-5 flex items-center gap-3">
        ...
        <div>
          <h3 className="text-h3 font-extrabold leading-tight text-navy">
            {t(method.title, provider.label)}
          </h3>
          {method.beneficiary && (
            <p className="text-caption text-navy/55">Bénéficiaire : {method.beneficiary}</p>
          )}
        </div>
      </div>

      <p className="mb-5 text-body leading-relaxed text-navy/70">{t(method.description)}</p>
      ...
      <p className="mt-4 text-xs leading-relaxed text-navy/50">{provider.instructions}</p>
    </article>
  );
};
